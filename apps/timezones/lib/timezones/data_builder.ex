defmodule Timezones.DataBuilder do
  @moduledoc false
  alias Timezones.DataLoader
  alias Timezones.PeriodBuilder
  alias Timezones.LeapSecParser
  require Logger

  # download new data releases, then parse them, build
  # periods and save the data in an ETS table
  def load_and_save_table do
    {:ok, content_length, release_version, tz_dir, modified_at} = DataLoader.download_new()
    current_version = Timezones.ReleaseReader.release_version()

    if release_version == current_version do
      # remove temporary tz dir
      File.rm_rf(tz_dir)

      Logger.info(
        "Downloaded timezones release from IANA is the same version as the version currently in use (#{
          current_version
        })."
      )
      ...
      do_load_and_save_table(content_length, release_version, tz_dir, modified_at)
    end
  end

  defp do_load_and_save_table(content_length, release_version, tz_dir, modified_at) do
    ets_table_name = ets_table_name_for_release_version(release_version)
    table = :ets.new(ets_table_name, [:bag, :named_table])
    {:ok, map} = Timezones.BasicDataMap.from_files_in_dir(tz_dir)
    ...
    :ets.insert(table, {:leap_sec_data, leap_sec_data(tz_dir)})
  ...
  defp leap_sec_data(tz_dir), do: LeapSecParser.read_file(tz_dir)

  def ets_file_name_for_release_version(release_version) do
    "#{release_dir()}/#{release_version}.v#{Timezones.EtsHolder.file_version()}.ets"
  end
