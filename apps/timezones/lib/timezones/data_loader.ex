defmodule Timezones.DataLoader do
  @moduledoc false
  ...
  def download_new(url \\ @download_url) do
    Logger.debug("Timezones downloading new data from #{url}")
    ...
    Logger.debug("Timezones data downloaded. Release version #{release_version}.")
  ...
  def latest_file_size(url \\ @download_url) do
    ...
    case latest_file_size_by_head(url) do
      ...
      _ ->
        Logger.debug("Could not get latest timezones file size by HEAD request. Trying GET request.")
  ...
  defp data_dir, do: Timezones.Util.data_dir()

  defp http_client() do
    Application.get_env(:timezones, :http_client, Timezones.HTTPClient.Hackney)
  end
end
