defmodule Timezones.EtsHolder do
  @moduledoc false

  require Logger
  use GenServer
  alias Timezones.DataBuilder
  alias Timezones.Util
  ...
  def handle_cast(:new_release_has_been_downloaded, state) do
    ...
    if state != new_release_name do
      Logger.info("Timezones has updated the release from #{state} to #{new_release_name}")
  ...
  defp delete_ets_table_for_version(release_version) do
    Logger.debug("Timezones deleting ETS table for version #{release_version}")
  ...
  defp delete_ets_file_for_version(release_version) do
    Logger.debug("Timezones deleting ETS table file for version #{release_version}")
  ...
  defp set_current_release(release_version) do
    # Logger.debug "Timezones setting current release version to #{release_version}"
  ...
  defp make_sure_a_release_is_on_file do
    ...
    cond do
      release_files() == [] and Util.custom_data_dir_configured?() ->
        Logger.info("No timezones release files found in custom data dir. Copying release file from timezones priv dir.")
        copy_release_dir_from_priv()
      release_files() == [] and not Util.custom_data_dir_configured?() ->
        Logger.error("No timezones release files found!")
      true ->
        nil
    end
  end
  ...
  defp copy_release_dir_from_priv() do
    custom_destination_dir = Timezones.Util.data_dir() <> "/release_ets"
    priv_release_ets_dir = Application.app_dir(:timezones, "priv") <> "/release_ets"
  ...
  defp release_dir do
    Timezones.Util.data_dir() <> "/release_ets"
  end

  @doc """
  Returns the file version number used by the current version of Timezones for the ETS files.
  """
