defmodule Timezones.TimeZoneDatabase do
  ...
  def time_zone_period_from_utc_iso_days(iso_days, time_zone) do
    ...
    case Timezones.periods_for_time(time_zone, gregorian_seconds, :utc) do
      [period] ->
  ...
  def time_zone_periods_from_wall_datetime(ndt, time_zone) do
    ...
    case Timezones.periods_for_time(time_zone, gregorian_seconds, :wall) do
      [period] ->
  ...
  defp gap_for_time_zone(time_zone, gregorian_seconds) do
    # Gap in wall time
    case Timezones.periods(time_zone) do
      {:error, :not_found} ->
        {:error, :time_zone_not_found}
  ...
  @doc !"""
       Takes a time_zone period in the format returned by Timezones 0.1.x and 0.5.x
       and returns one of the TimeZoneDatabase.time_zone_period type.
       """
  @spec old_tz_period_to_new(Timezones.time_zone_period()) ::
