defmodule Timezones do
  @moduledoc """
  The Timezones module provides data from the IANA tz database. Also known
  ...
  def zone_list, do: Timezones.ReleaseReader.zone_and_link_list()

  @doc """
  Like zone_list, but excludes aliases for zones.
  """
  @spec canonical_zone_list() :: [Calendar.time_zone]
  def canonical_zone_list, do: Timezones.ReleaseReader.zone_list()
  ...
  def zone_alias_list, do: Timezones.ReleaseReader.link_list()

  @doc """
  Takes the name of a zone. Returns true if zone exists. Otherwise false.

      iex> Timezones.zone_exists? "Pacific/Auckland"
      true
      iex> Timezones.zone_exists? "America/Sao_Paulo"
      true
      iex> Timezones.zone_exists? "Europe/Jersey"
      true
  """
  ...
  Otherwise false.
      ...
      iex> Timezones.canonical_zone? "Europe/London"
      true
      iex> Timezones.canonical_zone? "Europe/Jersey"
      false
  """
  ...
  Otherwise false.
      ...
      iex> Timezones.zone_alias? "Europe/Jersey"
      true
      iex> Timezones.zone_alias? "Europe/London"
      false
  """
  ...
  Returns a map of links. Also known as aliases.
      ...
      iex> Timezones.links()["Europe/Jersey"]
      "Europe/London"
  """
  @spec links() :: %{Calendar.time_zone => Calendar.time_zone}
  def links, do: Timezones.ReleaseReader.links()
  ...
  def zone_lists_grouped, do: Timezones.ReleaseReader.by_group()

  @doc """
  Returns IANA tz release version as a string.

  Example:

      Timezones.iana_version
      "2014i"
  """
  @spec iana_version() :: String.t
  def iana_version, do: Timezones.ReleaseReader.release_version()
  ...
  ## Example
      ...
      iex> Timezones.periods("Europe/Madrid") |> elem(1) |> Enum.take(1)
      ...
      iex> Timezones.periods("Not existing")
      {:error, :not_found}
  """
  @spec periods(Calendar.time_zone) :: {:ok, [time_zone_period]} | {:error, atom()}
  def periods(zone_name) do
    {tag, p} = Timezones.ReleaseReader.periods_for_zone_or_link(zone_name)
    case tag do
      :ok ->
  ...
  ## Examples
      ...
      iex> Timezones.periods_for_time("Asia/Tokyo", 63587289600, :wall)
      ...
      iex> Timezones.periods_for_time("Europe/Copenhagen", 63612960000, :wall)
      ...
      iex> Timezones.periods_for_time("Europe/Copenhagen", 63594816000, :wall)
      []
  """
  ...
  defp possible_periods_for_zone_and_time(zone_name, time_point, time_type) when time_point >= @point_from_which_to_use_dynamic_periods do
    if Timezones.FarFutureDynamicPeriods.zone_in_30_years_in_eternal_period?(zone_name) do
      periods(zone_name)
    else
      link_status = Timezones.ReleaseReader.links() |> Map.get(zone_name)
      if link_status == nil do
        Timezones.FarFutureDynamicPeriods.periods_for_point_in_time(time_point, zone_name)
      else
  ...
  defp possible_periods_for_zone_and_time(zone_name, time_point, time_type) do
    {:ok, periods} = Timezones.ReleaseReader.periods_for_zone_time_and_type(zone_name, time_point, time_type)
    mapped_periods = periods
    |> Enum.sort_by(fn {_, from_utc, _, _, _, _, _, _, _, _} -> -(from_utc |> Timezones.ReleaseReader.delimiter_to_number) end)
    |> Enum.map(
  ...
  ## Example
      ...
      iex> Timezones.leap_seconds_with_tai_diff() |> Enum.take(2)
  ...
  def leap_seconds_with_tai_diff do
    leap_seconds_data = Timezones.ReleaseReader.leap_sec_data()
  ...
  ## Example
      ...
      iex> Timezones.leap_seconds() |> Enum.take(2)
      [{{1972,  6, 30}, {23, 59, 60}},
       {{1972, 12, 31}, {23, 59, 60}}]
  """
  ...
  ## Example
      ...
      Timezones.leap_second_data_valid_until
      {{2015, 12, 28}, {0, 0, 0}}
  """
