defmodule TimezonesTest do
  ...
  test "Periods for UTC time in zone with DST around DST changes" do
    ...
    assert Timezones.periods_for_time("Europe/Copenhagen", gregorian_seconds_point_in_time, :utc) ==
             [
               %{
                 from: %{
  ...
  test "Periods for wall time just after DST changes" do
    ...
    assert Timezones.periods_for_time("Europe/Copenhagen", gregorian_seconds_point_in_time, :wall) ==
             [
               %{
                 from: %{
                   standard: 63_707_911_200,
  ...
  test "Get periods for point in time far away in the future. For all timezones." do
    ...
    |> Enum.map(fn(zone) -> Timezones.periods_for_time(zone, point_in_time, :wall) end)
  end
  ...
  test "Get periods for point in time far away in the future. For all timezones except Moroccan ones." do
    ...
    |> Enum.map(fn(zone) -> Timezones.periods_for_time(zone, point_in_time, :wall) end)
    Timezones.zone_list() |> Enum.map(&Timezones.periods_for_time(&1, point_in_time, :wall))
  end
  ...
  test "time for going on DST should be the same in the far future for zones without changes" do
    ...
    p1 = Timezones.periods_for_time(zone_name, feb_first_close, :wall) |> hd
    p2 = Timezones.periods_for_time(zone_name, feb_first_in_a_long_time, :wall) |> hd
  ...
  test "non existing time zone" do
    ...
    assert Timezones.periods_for_time(zone_name, gregorian_seconds, :utc) == []

    assert Timezones.periods_for_time(zone_name, gregorian_seconds, :wall) == []
  end
  ...
  test "test that atom count does not increase by ~1000 when doing query of 1000 zone names" do
    ...
    time_zone_names
    |> Enum.map(&Timezones.periods_for_time(&1, gregorian_seconds, :utc))

    time_zone_names
    |> Enum.map(&Timezones.periods_for_time(&1, gregorian_seconds, :wall))
